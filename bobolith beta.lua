print("bobolith loaded $$$")
print("uid: 666")
print("by: robpro#7420")
set_fps_cap("999999999")
print("fps unlocked")
rconsoleprint("good games 👋 ")
...
local cfglocation = "bobolithcfg/" 
makefolder("bobolithcfg") 
...
makefolder("bobolithlua") 

local allluas = {} 

for _,lua in pairs(listfiles("bobolithlua")) do 
	local luaname = GSUB(lua, "bobolithlua\\", "") 
	INSERT(allluas, luaname) 
end 

RunService.RenderStepped:Wait() 

local gui = library:New("bobolith") 
local by = library:New("by robpro#7420") 
...
luascripts:Element("Button", "load", {}, function() 
	loadstring(readfile("bobolithlua\\"..values.luas["lua scripts"].lua.Scroll))() 
end) 
...
chat:Element("Toggle", "chat spam", nil, function(tbl) 
	if tbl.Toggle then 
		while values.misc.chat["chat spam"].Toggle do 
			game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat.type.Dropdown == "Winning" and "Keep missing ft. bobolith" or "bobolith is like Monolith, $$$Miss$$$", false, "Innocent", false, true) 
			wait(values.misc.chat["speed (ms)"].Slider/1000) 
			 
		end 
	end 
end) 
chat:Element("Dropdown", "type", {options = {"Winning", "bobolith"}}) 
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current) 
	...
	if values.misc.chat["kill say"].Toggle then 
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "Everything > bobolith" and values.misc.chat["message"].Text or "pip", false, "Innocent", false, true) 
	end 
end) 
